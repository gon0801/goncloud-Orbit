...
difieren, gana el plan y se pausa a corregir este runbook (fila Atores 9).
...
0.1 Define una vez por terminal (del runbook FABRICA 02 D.1, DEPLOY.md):
...
Ventana de deploy: 09:30-15:00 UTC o despues de 16:00 UTC, excepto 13:00-13:20
UTC (margen del runbook sobre la corrida de precios 13:10), y lejos de
05:00-07:20 y 08:40 (runbook FABRICA 02).
...
| Deploy | Deploy de `orbit-app-1` | Hito correspondiente + B.2a resuelto + backup + rollback listo | A.4, B.4, C.6 |
| Live | Activar cortes nuevos en live | Shadow + riesgo aceptado (C.2b para C) | B.4, C.6 |
...
2. Deploy = DEPLOY.md paso 1 (copiar por `git archive` de `origin/master`,
   verificar md5, backup `app.bak-predeploy-<STAMP>`, build, `Recreated` con
   digest distinto) + precondiciones D.1.0 del runbook FABRICA 02 adaptadas
   (SHA en `$APROBADO`, backup nocturno presente). Citar la seccion, no
   copiarla.
3. Rollback de codigo = `git revert` del merge en rama `revert/<hito>`,
   ...
   Rollback de datos = solo el backup previo al deploy (runbook 4.1).
...
1. Registrar SHA productivo y modo efectivo (solo lectura). El server no es
   checkout git: el SHA se infiere comparando md5 de 3 testigos contra el
   candidato (metodo DEPLOY.md paso 1); si algun testigo difiere, el SHA
   productivo queda `unknown` y se anota el `app.bak` mas reciente:

```bash
CAND=$(git rev-parse origin/master)  # o el SHA que se va a verificar
for f in app/main.py app/ads/reports.py pyproject.toml; do
  echo "== $f"; ssh goncloud "md5sum /mnt/data/appdata/orbit/$f"
  git show $CAND:$f | md5sum
done
...
2. Elegir contencion aprobada por el dueno: (a) flag off por defecto en PR
   ...
   PAUSE nueva, o (b) revert de `beae9ec` por regla 3. El metodo elegido
   queda registrado y es el que H4/H5/H6 recomprueban. Sin evidencia de
   aislamiento, el deploy queda bloqueado (el plan lo exige).
...
- `fix/ads-report-timeout`: clasificar ANTES del deploy A.4 (toca la ingesta
  que A.4 observa). El lead compara sus 2 commits contra `master` (`43ddd93`
  ya entro por otra via): si aportan algo distinto, PR con review + CI + go
  de merge, y su merge precede al deploy A.4; si no, se abandona con registro
  en `docs/evidencia/ads-proteccion-01/H3/reporte-timeout.md`.
...
3. A.4 deploy (go de deploy): regla 2 + B.2a resuelto + aislamiento B.2
   ...
   Observacion 7 dias naturales: cierra con
   (i) exito principal del dia en /salud >=3 de 7 dias y cero falsos
   recovery, y (ii) si hubo fallo real, aviso + recovery observados.
   Sin fallo real en 7 dias, cierra como "desplegado, recovery pendiente de
   fallo real" con fila de seguimiento abierta (no bloquea H5/H6; C.6 lo
   registra como riesgo).

### H5 — B.4 (PAUSE sin cooldown: deploy, shadow, luego live)

El shadow corre en produccion (los goals viven ahi); el deploy va primero.

1. Deploy (go de deploy, cubre deploy + puesta en `shadow` de los goals
   afectados, registrada): regla 2 con la contencion B.2a vigente. Luego 5
   ciclos Ads (5 dias): comparar candidatos PAUSE-nueva vs replay B.3.
   Parada si: (a) candidato no explicado por B.3, (b) el ciclo falla o no
   marca `done`, (c) mutacion real (cero `applied` nuevos en `apply_queue`:
   contar antes y despues del shadow). Evidencia por ciclo.
2. Go de live separado (cubre el flip a `live`, registrado). Rollback por
   regla 3 (revert `beae9ec` + 1 ciclo sin PAUSE-nueva).
...
4. C.6: deploy (go de deploy, cubre deploy + puesta en `shadow`,
   registrada) con el aislamiento off vigente; 5 ciclos con criterio de
   parada H5 + riesgo C.2b citado; go de live separado (cubre el flip,
   registrado); rollback por regla 3. Cierra con comparacion candidatos vs
   applies sin lookahead; registra "senal A.3 sin recovery observado" como
   riesgo si H4 cerro parcial.
...
1. Superficie C.4: pantalla `/propuestas` (alias `/cortes`, `app/ui.py`) +
   ...
   agregue; la UI consume el endpoint, regla 22) + aviso Telegram de
