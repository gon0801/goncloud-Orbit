...
**Corrida.** `salidas/CORRIDA.txt`: `estado: COMPLETA`, 2026-09-18T11:12:46Z a 2026-09-18T11:13:08Z UTC, `commit_del_repo: fdbebe8`, `consultas_con_cambios_sin_commitear: 0`. «Hoy» es la fecha UTC de esa corrida, **2026-09-18**. La ventana de 90 días es `[2026-09-18 − 90, 2026-09-18]` = `[2026-06-20, 2026-09-18]`. Cada tabla cita el `.sql` de `consultas/` que la produjo, y su salida literal está en `salidas/<mismo nombre>.txt`.
...
Desde la raíz del repo, con `ssh goncloud`. Reescribe `salidas/` y `salidas/CORRIDA.txt`, y aborta antes de tocar producción si alguna consulta contiene una palabra de escritura, una diagonal invertida, una sentencia que no empiece con `select` o `with` (`solo-select.py`: quita los comentarios `--` respetando los strings y rechaza todo `$`, todo comentario de bloque `/* */` y todo string sin cerrar, así que `commit;`, `END WORK;` o `do $$ … $$` no pasan y un `case … end` multilínea sí) o una palabra de control de transacción que cabe dentro de un `select` (`into`, `set_config` y demás) (un metacomando de `psql` como `\!` se ejecutaría en el servidor sin que `BEGIN READ ONLY` lo controle). Los dos candados se prueban sin tocar producción con:
