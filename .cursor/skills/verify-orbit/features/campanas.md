...
- **Filtrar.** Corre `curl -sS "$BASE/campanas?estado=ENABLED&procedencia=goal_plataforma"`. Status 200 y sigue mostrando `Campana A`. Corre `curl -sS "$BASE/campanas?estado=PAUSED"`: el HTML contiene `Ninguna campaña coincide con el filtro.` (semilla ENABLED).
