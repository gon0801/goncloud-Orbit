...
def test_informe_identifica_autor_y_padre_con_mantenedor_fijo(fake_catalog, monkeypatch, tmp_path):
    for autor in ("autor-alfa", "autor-beta"):
        parent = runs.prepare_run(
            case_id="codigo",
            model=autor,
            harness="terminal-autor",
            config={},
            attempt=1,
            output=tmp_path / "padres" / autor,
        )
        _deliver_and_grade(monkeypatch, parent)
        runs.extend_run(parent, tmp_path / "extensiones" / autor, model="mantenedor-fijo")
    texto = report.build_report(tmp_path / "extensiones")
    for autor in ("autor-alfa", "autor-beta"):
        parent_id = _json(tmp_path / "padres" / autor / "run.json")["run_id"]
        assert f"Autor padre: {autor} / terminal-autor" in texto
        assert parent_id in texto
