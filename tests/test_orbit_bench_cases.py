...
def test_depuracion_rechaza_umbral_de_clicks_historico_fijo(tmp_path: Path) -> None:
    mutante = _entrega(
        tmp_path,
        _correcto_depuracion().replace(
            'min_clicks = int(congelada["min_clicks"])', "min_clicks = 25"
        ),
    )
    assert grade("depuracion", mutante)["automatic_pass"] is False
