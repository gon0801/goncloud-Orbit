...
- COMPLETITUD: la unidad es la ENTIDAD: >= 7 fechas distintas de la entidad
  dentro de SU ventana (AgregadoMetricas.completa, evaluada por ventana: la
  de bids para 2.2, la de cortes para 2.3). Los terminos NO exigen fechas
  propias: la evidencia por termino la dan sus umbrales de clicks/cost;
  exigir 7 fechas por termino mataria los negativos long-tail.
...
_SQL_MAX_FECHA_ENTIDAD, _SQL_AGREGA_METRICAS, _SQL_TERMINOS_CORTES,
_SQL_WATERMARK_PLATAFORMA, _SQL_SYNC_PLATAFORMA.
"""
...
class VentanasEntidad:
    ...
    `terminos` vive SIEMPRE en la ventana de cortes (2.3)."""
...
# entidad pertenece a una sola plataforma).
...
def terminos_cortes(
    ...
    La ventana la deriva esta funcion desde decided_at: el llamador no puede
    pasarle por error la de bids."""
    max_fecha = _max_fecha(conn, ad_entity_id)
    if max_fecha is None:
        return ()
    window_end = fin_ventana_cortes(max_fecha, decided_at)
    filas = conn.execute(
        _SQL_TERMINOS_CORTES,
        (ad_entity_id, inicio_ventana(window_end), window_end),
    ).fetchall()
    return tuple(
        AgregadoTermino(
            ad_entity_id=fila[0],
