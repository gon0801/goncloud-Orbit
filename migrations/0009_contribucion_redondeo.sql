...
--  final; los CTEs internos siguen exactos, asi ratio_dia/cobertura no
--  cambian ni una milesima de precision).
--
--  Definicion = 0008 + ROUND en las 4 columnas computadas. Misma interfaz,
--  misma marca multilisting. Los valores publicados cambian a lo sumo en
--  el redondeo (trailing digits mas alla del 4o decimal).
--  Reversa: re-aplicar la 0008 (misma interfaz; OR REPLACE directo).
-- =============================================================================
...
SELECT s.platform,
       ...
       ROUND(s.revenue_same_sku_sum - s.cost_sum - c.cogs_sin_halo, 4) AS contrib_sin_halo,
       ROUND(s.ad_revenue_sum - s.cost_sum - c.cogs_con_halo, 4)       AS contrib_con_halo,
       c.rango_invertido,
       c.fx_source,
