...
| 1.2 | **Lead — review, merge, deploy con BIDS 01 · 1.5 y verificación**: conteos de `ad_entity`/`ad_entity_state` por plataforma y kind antes y después de un `ingest structure` post-deploy (idénticos); `tools/snapshot_listas.py` sigue corriendo en el contenedor; AppFlowy. `[tdd:skip:ops]` | Conteos idénticos en la evidencia; snapshot ok; AppFlowy Done | 1.1 | cc:完了 [**2026-09-03, lead**: desplegado en el mismo deploy que BIDS 01 y CAMPAÑA ACTIVA 01 · 1.6 (imagen 03379cf, md5 38/38 = master 9f60d7e). Verificado dentro del contenedor: `structure_api` y `structure_plan` importables y la fachada apuntando al mismo objeto (`structure.listar_todo is structure_api.listar_todo`), los **7 `_SQL_*` en la fachada**. **Prueba en vivo**: `python -m app.cli ingest structure` (ingest_run 73, ok=True, rows_written=16,800, **entidades_nuevas=0**) y conteos de `ad_entity` por plataforma/kind **IDÉNTICOS** antes y después (MX 167/188/2645/861/7709, US 74/79/1336/549/4818; `ad_entity_state` 18,426): el corte no cambió el resultado del sync] |
