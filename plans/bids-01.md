...
| 2.3 | **Reconciliación del ledger** (H3): `--reconciliar` que cruza `planeado`/`failed` contra el LIST real de Amazon y promueve a `applied` lo que ya está `ARCHIVED`; `--reponer` lo incluye. `[tdd:required]` | Rojo: fila `failed` cuya keyword está ARCHIVED en Amazon se recupera y es reponible | - | cc:完了 reconciliación del ledger con --reconciliar (PR #158, CI verde) |
...
| 2.4 | **Autorizar por identidad, no por conteo** (H4): `--ids-file` o hash ordenado de `external_id` del ensayo; el go aborta si el conjunto cambió. `[tdd:required]` | Rojo: mismo N con un conjunto distinto → aborta | - | cc:完了 autorización por huella del conjunto con --huella (PR #159, CI verde) |
...
| 2.5 | **La reposición no gasta** (H5): `--reponer` crea en **PAUSED** (precedente `archivar.py`), exige `--go`, verifica bid y campaña en el readback, y registra el `external_id` nuevo. Documentar que el archivo de Amazon es **irreversible en la práctica**. `[tdd:required]` | Rojo: la repuesta nace PAUSED con bid y campaña verificados | - | cc:完了 reponer en PAUSED con go y readback de bid+campaña (PR #160, CI verde) |
| 2.6 | **/inertes muestra la puerta de antigüedad y los lotes** (decisión del dueño 2026-09-04: «hay que cerrar el hueco»): la pantalla no distinguía «en espera» (puerta 30d de 2.1) de archivable ni mostraba el ledger de lotes. Sin migración: JOIN a `ad_entity` por `first_seen_at` (precedente del tool), `en_espera` espejo EXACTO del predicado del tool con su default N=30 (si ese default cambia, el 30 del SQL se toca — declarado en comentario), `archivable_desde` informativo, y tarjeta «Lotes de archivado» con conteos por estado desde `keyword_archivo_manual` (vacía → estado vacío, regla 3). `[tdd:required]` | Rojo: hoja con `first_seen_at` de hoy → `en_espera=true` y `archivable_desde=hoy+30`; una pisada a 60d → `false`; lotes sembrados → conteos correctos; tabla vacía → `[]`. Rojo demostrado contra el código sin el cambio (`KeyError: 'lotes'`) | - | cc:完了 endpoint + template + tests contra Postgres real (PR, CI verde) |
