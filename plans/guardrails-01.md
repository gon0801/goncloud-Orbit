...
| 1.4 | Candado de frescura del contexto de chat (aprobado por el dueño 2026-08-23; la alternativa de un GitHub Action que REDACTA estado se rechazó — contenido sin review a master contradice la disciplina del repo; este candado solo REVISA): en CI de PRs, si el diff agrega markers `cc:完了` a `plans/*.md` sin tocar `docs/CHAT-CONTEXT.md`, falla con instrucción. Script `tools/check_chat_context_fresh.py` con lógica pura testeable. Mergeado directo a master (PR #14, commit c1d2b4e) porque el Project de Claude Chat sincroniza desde master. `[tdd:required]` | 5 tests de la lógica pura incl. el caso que muerde (regla 9: completada sin contexto → exit 1) y línea borrada (no dispara); paso en quality.yml solo en pull_request; CI verde en PR #14 | 1.1 | cc:完了 |
