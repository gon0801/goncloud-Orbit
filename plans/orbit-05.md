...
>
> **ENMIENDA DEL DUEÑO A LA CADENCIA DE RAMPA — 2026-09-03, literal «no
> podemos ser ultra conservadores… un test es suficiente»**: la regla de
> «duplicación cada 48h sanas» se sustituye por «subida por decisión del
> dueño tras un ciclo sin incidentes» (APPLY.md §5.5 reescrito, no
> ignorado). Aplicada el mismo día con 2 ciclos live limpios (33/34 y
> 35/36: 40 mutaciones, 0 failed, 0 divergencias): caps 10→40 bids, 2→5
> pause, 5→15 negative, 2→5 harvest (config 12). Acordado además: si el
> ciclo del 2026-09-04 sale limpio, bids sube a 100 **permanente** (fusible,
> no freno) y los cortes se quedan quietos una semana.
>
...
| 2.1 | **Verificación adversarial TRIPLE** (ítem 9, sellado 6b): codex + grok + qwen reciben las primeras decisiones APLICADAS **SANEADAS** (decisión + inputs congelados + request/ack pasados por el scrub del repo + readback; JAMÁS tokens, headers, profile ids ni ids de cuenta — los datos comerciales (keyword, gasto, ventas, bid) sí viajan, como en todos los cross-reviews previos; codex plan r1: divulgación declarada en 事前確認), con muestra MX forzada (mínimo 5 MX si existen; si no hay MX aplicadas el día 1, se declara y se repite al primer día con MX). Cada uno recalcula contra las reglas selladas y busca divergencias entre decisión, request, ack y readback. Tope: 1 ronda; residuales declarados. `[tdd:skip:verificacion-adversarial]` | Tres reportes adjudicados por el lead; cero divergencia sin explicar; si hay bug → escalera a shadow y tarea de fix ANTES de seguir | 1.5 | cc:完了 [2026-09-02 23:41-00:40 UTC, lead. Expediente saneado con `tools/dossier_adversarial.py` (2.1a) corrido desde la máquina del lead por túnel SSH con el rol `orbit_read` (DEPLOY.md §túnel; contraseña solo en el entorno del proceso): 20 registros (10 US / 10 MX), 20/20 `replay_coincide`, 20/20 `verify_ok`, cero patrones de secreto (escáner de la herramienta + escaneo propio del lead). Tres revisores, UNA ronda (tope), prompt del lead `out/adversarial-2026-09-02/prompt_lead.md`: **codex** (`codex exec -s read-only`), **qwen** (`qwen -s -p`), **grok** (`grok -p --always-approve` con Write/Edit/git/ssh/docker negados; 3 intentos previos cayeron por flags del CLI, no por la revisión). **Veredicto de los TRES: «las 20 aplicadas son fieles a las reglas selladas», 0 divergencias decisión→request→ack→readback→replay** (cada uno recalculó las 20: ACoS con revenue completo, banda por multiplicación, bid esperado, ROUND_HALF_EVEN a centavos, ids del request/ack = external_id, readback, madurez, `data_observed_at ≤ decided_at`, goal scope 6/7 en 1995/1997, moneda/piso/techo por país, pause no aplicable). Adjudicación del lead en §Adjudicación 2.1 (hallazgos H1-H5: cero bugs del motor; 2 huecos de DISEÑO ya documentados como propuesta NO aprobada del 2026-08-26 → decisión del dueño; 1 residual de docs; 2 mejoras de la herramienta). Residual «orden bajo cap no verificable con el dossier» CERRADO por el lead con SELECT sobre las 123 decisiones de 33/34: las 10 aplicadas por plataforma son EXACTAMENTE el top-10 del orden sellado (banda → `cortes.cost` DESC); 1 US + 6 MX `banda_mas_15` decididas quedaron fuera por prioridad (diseño). Reportes en `out/adversarial-2026-09-02/reporte_{codex,qwen,grok}.md`] |
...
| 2.3 | **Primer harvest live a mano** (sellado 6a): cuando el primer harvest salga de la cola (48h después del flip como mínimo): readback LIST en la campaña destino (MX = Arras Manual, terna goal 4; US = lo que el dueño decidió en preflight 1.6), identidad completa (keyword_text + match_type + ad group + bid), ledger sellado, y `keywords_campana_destino` sin duplicado. `[tdd:skip:ops]` | Keyword nueva visible en Amazon con el bid del goal; ledger `applied` con readback; cero duplicados; evidencia | 1.5 | cc:WIP [**2026-09-04: SALIÓ EL PRIMER CORTE DE LA HISTORIA.** El ciclo 37 encoló un **harvest** (`apply_queue` id 7, `pending_veto`, vence **2026-09-06 08:40:02 UTC**): término de búsqueda **«arras para boda cristiana»** de AU2 Category Phrase US, con **63 clics, 28.22 USD de gasto, 2 órdenes y 203.20 USD de ingreso → ACoS 13.9 %**, contra la regla `orders>=2 ∧ ACoS ≤ min(35 %, target)` con el target **19.5 del peldaño de margen**. Crearía la keyword EXACT en la campaña manual (`251723662158466`, ad group `522582072501798`, bid 0.68 USD). El dueño tiene 48 h para vetarlo en `/cortes`; si no, se aplica solo el 09-06 y ENTONCES el lead lo verifica a mano en Amazon (readback de la keyword creada, ledger sellado) **DESENLACE (2026-09-04): el dueño lo VETÓ POR ERROR de interfaz y el lead creó la keyword a mano.** Literal del dueño: «no no queria vetarlo queria mandarlo a harvest, osea es confuso» — la pantalla se llama «Cortes» y su único botón dice «vetar», así que una propuesta de CRECIMIENTO (63 clics, 28.22 USD, 2 órdenes, 203.20 USD, ACoS 13.9 %) se leyó como recorte. El veto es TERMINAL por diseño (0002 no tiene salida de `vetoed`) y bloquea re-decidir esa clave hasta `vence_el` 2026-10-04: **no se puede revertir, y el lead lo dijo en vez de intentar un atajo**. Para no perder el mes, el dueño ejecutó la creación manual por el camino SELLADO (`apply._cliente_reversa` + `crear_keyword_exacta`, dry-run primero, idempotente y con readback): **`keywordId` 260446590936405, EXACT, 0.68 USD, readback ENABLED**. El sync de estructura la adopta y el motor le administra la puja desde el próximo ciclo. **Causa raíz atacada**: `plans/cortes-ui-01.md` (la pantalla deja de mentir). La tarea 2.3 sigue abierta: falta un harvest que el motor aplique SOLO, de punta a punta, para cerrarla] — **READBACK POST-MORTEM 2026-09-04 ~18:00 UTC (lead)**: la keyword manual vive en Amazon verificada por LIST (`/sp/keywords/list`, keywordIdFilter, read-only desde el contenedor): keywordId 260446590936405, EXACT, «arras para boda cristiana», campaignId 251723662158466, adGroupId 522582072501798, bid 0.68 USD, ENABLED. Cero duplicados vivos del mismo texto+EXACT en la campaña destino (LIST por campaignIdFilter: solo ella). La fila de la cola (id 7) quedo `vetoed` (terminal); la creacion manual no paso por el ledger de harvest (camino sellado de reversa, no por apply_harvest) — la evidencia de esa creacion es este plan + CHAT-CONTEXT. Sigue abierta: el DoD exige un harvest que el MOTOR aplique de punta a punta.
| 2.4 | **Monitoreo 48h + hábito de veto** (ítem 8, sellado 5): digest diario por Telegram, `/cortes` revisado por el dueño al menos una vez al día (registrado), quota `used/cap` en la superficie de preflight 1.4-1.5, alertas de cap saturado, cero `failed` sin analizar; anotar en el runbook que el ACoS de día 1 no se evalúa (sellado 7). `[tdd:skip:ops]` | Dos días consecutivos con: ciclos `done`, quota dentro de cap, cola sana (sin `applying` colgado), dueño con al menos una visita a `/cortes` por día registrada | 1.5 | cc:完了 [**Día 1 (2026-09-03)**: ciclos 35 US / 36 MX `done` live (primer ciclo del cron post-flip y post-deploy de CAMPAÑA ACTIVA 01); 10+10 bids aplicados, 0 failed, 0 divergentes, quota 10/10 = cap; cola sana (0 live, 0 `applying`); ningún corte encolado aún (nada que vetar en `/cortes`: la visita del dueño no aplica mientras la cola esté vacía — se registra cuando haya cortes); digest y aviso de cap enviados sin NOTA de fallo. ACoS del día 1 no se evalúa (sellado 7). **Día 2 (2026-09-04) LIMPIO**: ciclos 37 US / 38 MX `done` live, **30 bids aplicados** (7 US + 23 MX), **0 descartados, 0 divergentes, 0 fallos, 0 intentos sin sello**; quota 7/40 y 23/40 (los caps nuevos ni se rozan: la guarda `entidad_inerte` sacó 168 US / 176 MX del ciclo y ESA es la palanca real, no el cap); cooldown 19 + 20 (nada repetido); `campana_no_enabled` 1594/3111 (bajó 3 en US y 4 en MX = las hojas de AGM2M, ya reactivada); primer ciclo con el target por margen gobernando (ver `plans/orbit-06.md` 2.4). **48 h consecutivas sin incidentes**: la condición de la enmienda de cadencia queda satisfecha. **2026-09-03 (mismo día 1), por enmienda del dueño**: deploy único con BIDS 01 completo + CAMPAÑA ACTIVA 01 · 1.6 + ESTRUCTURA 01 (migraciones 0013/0014 primero, imagen 3673dd5→03379cf, md5 38/38 = master 9f60d7e, health 200, `/inertes` 200, cero errores; verificado dentro del contenedor) y **caps subidos a 40/5/15/5** (config 12, append-only; escalera y targets intactos; rige desde el ciclo del 09-04). Estado tras el deploy: cola live 0, intentos sin sello 0, 4 goals live] — **CIERRE 2026-09-04 (lead)**: DoD cumplido — dia 1 (09-03) y dia 2 (09-04) consecutivos con ciclos `done`, quota dentro de cap y cola sana (la visita del dia 1 se declaro n/a por cola vacia; la del dia 2 quedo registrada: el veto del harvest). Verificado ademas el dia 3 en prod: ciclos 37/38 live/done, quota 7/40 US y 23/40 MX, cero `applying` colgado, cola solo con filas terminales (1 live vetoed; shadow: 3 discarded + 2 vetoed). |
...
## Adjudicación 2.1 — verificación adversarial triple (lead, 2026-09-03 00:40 UTC)

| # | Fuente | Hallazgo | Verificación del lead | Decisión |
|---|---|---|---|---|
| H1 | codex (media) · qwen (media) · grok (media) | 8 de las 10 MX son −12% sobre entidades con gasto y **cero ventas**: con `ad_revenue=0`, `cost > 1.15×target×0` es siempre verdadero y −25% exige `orders ≥ 1` (`bid.py`), así que cero-ventas nunca baja más de −12%; PAUSE tampoco alcanza (clicks/cost bajo el umbral congelado). Fiel al motor sellado; la spec `docs/superpowers/specs/2026-08-26-banda-cero-ventas-design.md` (opción A) sigue como PROPUESTA no aprobada. grok añade: aun con la opción A, ninguna de las 8 calificaría (todas `cost < 500 MXN`) | Confirmado contra `bid.py` y la cabecera de la spec | NO es divergencia. **Decisión pendiente del dueño**: aprobar/rechazar la opción A (nace plan BIDS 01) o dejarlo como residual explícito |
| H2 | qwen (baja) · grok (media) | **Ajustes inertes**: 4 de las 20 se decidieron con ventanas que terminan en junio (`window_end` = max(metric_date) − 3d de ESA entidad). 1989/2104 son las de campañas PAUSED (ya resuelto en CAMPAÑA ACTIVA 01); **2059 (`arras matrimoniales`, AGMX Category Exact) y 2078 (`arras de boda plata`, AC Category Exact) están en campañas ENABLED sin tráfico desde junio**: bajarles el bid no cambia nada y gastó 2/10 cupos MX del día 1. Opción C de la misma spec (guarda `entidad_inerte`) no aprobada | Confirmado con el dossier (window_end 2026-06-24 y 2026-06-20) | NO es divergencia. **Decisión pendiente del dueño**: aprobar la guarda de entidad inerte (opción C) o aceptar el gasto de cupos en entidades sin tráfico |
| H3 | codex (baja) · grok (residual) · qwen (no verificable) | Orden bajo cap: sellado (orbit-04 sellado 8) y respetado en el orden del ledger, pero no demostrable contra las 103 descartadas con el dossier. grok: `_clave_orden` usa `ventanas.cortes.cost` mientras la banda se calcula sobre la ventana de bids; APPLY.md dice «costo de la ventana» sin decir cuál | **CERRADO por SELECT del lead** sobre las 123 decisiones de 33/34: `aplicadas_fuera_top10 = 0` y `no_aplicadas_en_top10 = 0` en ambas plataformas (orden banda → `cortes.cost` DESC). El set no cambia si se ordenara por `bids.cost` (solo el orden interno) | Residual de DOCS: APPLY.md aclara que el costo del ranking es el de la ventana de CORTES (corregido en este PR). Mejora de la herramienta: incluir las descartadas por tope con su banda y costo |
| H4 | grok (baja) | El dossier no congela `ad_group.status` (solo el de la campaña y la hoja) | Confirmado (`CLAVES_AD_GROUP = id, name`) | Mejora de la herramienta (2.1a): agregar `status` del ad group y las filas de `apply_quota_state` |
| H5 | qwen (baja) | 2104: `data_observed_at` ≠ `bids.observed_at_max` | Es la regla sellada `LEAST(decided_at, max(obs directo, evidencia del grupo))` (`cycle._sello_bitemporal`, CORTES 01 1.2 / qwen r2); los tres revisores coinciden en que no afecta la decisión | DESCARTADO como hallazgo; queda documentado aquí |

Cero divergencias entre decisión, request, ack, readback y replay en las 20;
cero bugs del motor. La escalera NO baja a shadow. Lo que se materializó en
vivo son dos huecos de diseño ya conocidos y documentados como propuesta el
2026-08-26 (H1, H2): pasan al dueño como decisión, no como fix.
...
4. Cero ventas (H1) y ajustes inertes (H2) siguen vigentes en el motor hasta
   que el dueño decida sobre la propuesta del 2026-08-26 (opciones A y C).
