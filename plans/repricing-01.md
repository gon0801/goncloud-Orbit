...
| A.4 | [stage:verificacion] [lane:release] [tdd:skip:sonda] **Sonda de escritura y reversa con ids reales** (dueño con `!`, go literal; lead lee): un producto controlado MX; `+0.01` por `precio_write.py` y reversa por la herramienta. Sella la forma exacta del parche. **Prerrequisitos (v1.3), que la v1.2 no nombraba**: la 0039 aplicada y las claves sembradas (**D.0**); un goal `live` con go literal para el producto controlado, sembrado con `tools/precio_goal.py` (**A.1**); y una fila de `precio_decision` `subir` en `live` con la cuenta completa (los diez importes, `p_actual` = precio vivo, `p_aplicado` = `p_actual + 0.01`), **insertada a mano como `app_decide`** porque la corrida (A.5) todavía no existe — el INSERT literal va en la evidencia. La sonda es iterativa: se propone el cuerpo inyectando `construir_cuerpo=` a `cambiar_precio`, y cuando Amazon lo acepta y el GET lo confirma, se sella en un PR `fase<N>/a4-forma-parche` que escribe el cuerpo en `construir_cuerpo_parche`, cambia `FORMA_PARCHE` y ajusta los tests que fijan `pendiente_sonda`; ese PR es la única edición de código de A.4 y lleva el loop normal. | `docs/evidencia/repricing-01/A.4/` en orden temporal: observación del día anterior = `P`; fila `pendiente` con `enviado_at` antes del ack; ack con `submissionId`; GET con precio `P + 0.01`, `submissionId` y observación posterior de Pricing con `observed_at > enviado_at` (el `summaries.lastUpdatedDate` de Listings Items no fecha la oferta en estos listings; ver E/A.4); ack y GET de la reversa; observación del día siguiente = `P`; **control negativo** (otro listing propio sin cambio conserva su precio); PR de sello mergeado con `git grep -n pendiente_sonda -- app/spapi/precio_write.py` **sin línea de código** (solo el comentario ancla de l.59–61 puede quedar), `FORMA_PARCHE` sellado y `construir_cuerpo_parche` devolviendo el cuerpo real; **sin E/A.4 no hay D.2** | A.1, A.3, D.0 | cc:完了 [2026-09-22: cambios 2/3/4 y reversas 5/6/7 confirmados por observación de Pricing; precio original en 1213/1284/1295 el 21 y 22-sep; control 1253 intacto. El GET y `submissionId` prueban aceptación inmediata, `observed_at` posterior prueba lectura externa; `summaries.lastUpdatedDate` no avanzó y se retiró como criterio temporal. Forma del PATCH sellada en #319. Ver E/A.4/readback.md.] |
...
| D.1 | [stage:cierre-pr] [lane:release] [tdd:skip:validacion-entrega] **Despliegue en sombra MX (FBA)** (dueño con `!`): backup, la migración ya aplicada por D.0 (se verifica, no se repite), deploy por `git archive` + checksum + rebuild, cron instalado, goals sembrados en `shadow` para los productos que el dueño elija y **una corrida automática válida** leída por el lead después del refresco diario de insumos. Cinco corridas quedan como seguimiento no bloqueante. | E/D.1 con SHA y salidas: **3/3 goals vigentes evaluados** en la corrida automática; **el recuadro de cobertura cuadra** con las activas de MX de la fuente canónica vigente, con las 342 identidades de `listing` al lado y activas del bridge declaradas desconocidas; cuenta de los tres productos reproducida a mano; trayectoria y cooldown cubiertos por las pruebas automatizadas existentes; cero goals `live`, cero cambios aplicados y ningún PATCH salvo A.4 | R.1, D.0 | cc:完了 [2026-09-23: el dueño autorizó D.1 con `!`; `f301cf4` desplegado el 20-sep con respaldo y checksums, cron único en `gon`, goals 6/7/8 en `shadow`. Corridas automáticas del 21 y 22-sep: 3/3 evaluados cada día a las 13:10 UTC, `mantener(cooldown)` por cambio A.4 a 2 y 3 días frente a 7 configurados; cero goals live y cero cambios nuevos. Recuadro 260 = 3 + 102 + 155 + 0 en ambos días. La corrida manual temprana del 20-sep con `costo_desactualizado` no cuenta. Cinco días siguen como seguimiento no bloqueante. E/D.1.] |
...
| AC11 | Una corrida automática en sombra después del refresco diario; cinco días de seguimiento no bloqueante | 3/3 goals evaluados; cuenta reproducida a mano en los 3 productos; cero goals `live`, cero cambios aplicados y cero PATCH | D.1 |
...
- **A.4 cerrada el 2026-09-22**: los cambios 2/3/4 y las reversas 5/6/7
  están `confirmado` por observación; Pricing del 21 y 22-sep muestra los tres
  precios originales y el control 1253 intacto. La forma
  `listings_items_purchasable_offer_v1` se selló en #319. Evidencia en
  `docs/evidencia/repricing-01/A.4/`.
- **D.1 cerrada el 2026-09-23**: `f301cf4` desplegado el 20-sep con respaldo;
  cron en `gon` y goals `shadow` 6/7/8. Corridas automáticas del 21 y 22-sep
  evaluaron 3/3 cada día, mantuvieron por cooldown y no aplicaron cambios.
