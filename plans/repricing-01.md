...
Version: 1.3, 2026-09-17 UTC. Estado: **plan sellado sobre el spec v1.3
...
abierta por E.0). Base: `origin/master` `6e8d094`. **Qué cambia en la v1.3**
(revisión del lead de la Fase 8 contra el código, 2026-09-17): fila **D.0**
...
| D.0 | [stage:cierre-pr] [lane:release] [tdd:skip:ops] **Base de producción lista para la sonda** (dueño con `!`; lead lee; v1.3): aplicar `migrations/0039_precio.sql` con el procedimiento sellado de `docs/DEPLOY.md` §«Migración 0039» (backup `--schema-only` completo con marcador de cierre, `psql --single-transaction -v ON_ERROR_STOP=1`, verificación como `orbit_read` y desde el contenedor app); sembrar en `config_version.settings` las claves `precio_*` de la tabla de umbrales con su valor inicial, **salvo las cinco de envío** (`precio_envio_*`), que se siembran cuando E.2 selle y E.3 las lea; ningún goal, ninguna decisión, ningún precio. | `docs/evidencia/repricing-01/D.0/` con: el backup nombrado; la salida de la migración; readback como lector: las cinco tablas `precio_*` existen y tienen **cero filas**, `listing_id_platform_key` existe, `apply_cap_de_config('precio:amazon_mx')` devuelve `5`; desde el contenedor app, `leer_config(settings_vigentes)` no levanta `ValueError` y `apply_quota_state` para `precio:amazon_mx` nace con `cap = 5` en un `BEGIN … ROLLBACK`; la lista literal de claves sembradas con su valor; `docs/DEPLOY.md` dice que la 0039 **ya está aplicada** con fecha | A.0, A.2 | cc:TODO |
...
| D.0 | `docs/evidencia/repricing-01/D.0/`, `docs/DEPLOY.md` (solo la línea «aplicada el …» de §0039) | Producción solo por el dueño con `!`; el lead lee |
...
- **Del dueño, en paralelo y en este orden**: E.2 (los números de E.1 están en
  los hechos 20–22), D.0 (0039 + claves, con `!`), goal `live` del producto
  controlado (con la herramienta de A.1) y A.4. A.5 arranca cuando A.1 esté en
