...
| D.1 | [stage:cierre-pr] [lane:release] [tdd:skip:validacion-entrega] **Despliegue en sombra MX (FBA)** (dueño con `!`): backup, la migración ya aplicada por D.0 (se verifica, no se repite), deploy por `git archive` + checksum + rebuild, cron instalado, goals sembrados en `shadow` para los productos que el dueño elija y **una corrida automática válida** leída por el lead después del refresco diario de insumos. Cinco corridas quedan como seguimiento no bloqueante. | E/D.1 con SHA y salidas: **3/3 goals vigentes evaluados** en la corrida automática; **el recuadro de cobertura cuadra** con las activas de MX de la fuente canónica vigente, con el bridge mostrado al lado; cuenta de los tres productos reproducida a mano; trayectoria y cooldown cubiertos por las pruebas automatizadas existentes; cero goals `live`, cero cambios aplicados y ningún PATCH salvo A.4 | R.1, D.0 | cc:WIP — criterio reducido por aprobación del dueño: una corrida automática válida bloquea el cierre; cinco días son seguimiento. Desplegado `f301cf4` el 2026-09-20 UTC, backup y checksum completos, cron 13:10 UTC instalado, goals shadow 6/7/8 para listings 1213/1284/1295. El preflight de 03:xx UTC fue anterior al refresco de costos de 08:15: tres `no_evaluado(costo_desactualizado)`, cero cambios y cero PATCH; no cuenta. Primera corrida válida esperada el 21-sep. Cobertura viva: 260 canónicas y 342 identidades del bridge; ver `docs/evidencia/repricing-01/D.1/LEEME.md`. |
...
- **D.1 en curso desde 2026-09-20 UTC**: `f301cf4` desplegado, cron 13:10 UTC y
  goals `shadow` 6/7/8 para listings 1213/1284/1295. El preflight anterior al
  refresco diario de costos quedó como `costo_desactualizado` y cero PATCH; las
  la corrida automática válida queda para el 21-sep. Cinco días quedan como
  seguimiento no bloqueante. Evidencia en
  `docs/evidencia/repricing-01/D.1/LEEME.md`.
