...
| 3.2 | Goals amigables: `app.cli goals set` + endpoints write (target/enabled/floor/ceiling/harvest_*) con auth — UPDATE con updated_at explícito, config=fila nueva, una implementación CLI/endpoint. Desbloquea dashboard-01 Phase 3. `[tdd:required]` | Tests: edición visible al ciclo siguiente con rastro; updated_at (regla 9); harvest_* respeta all-or-nothing del CHECK; sin auth rechazado; camino único | 3.1 | cc:完了 [9203d71] |
