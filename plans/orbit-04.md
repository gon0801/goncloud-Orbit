...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 4.1 | Deploy endurecido: env por servicio (DSN admin solo en app), non-root con permisos de secrets resueltos, **aplicar 0002 en goncloud** (runbook DEPLOY.md actualizado, backup previo del schema). `[tdd:required]` | Candados de compose/Dockerfile demostrados fallando; 0002 aplicada y verificada en vivo (regla 8: SELECT de las tablas nuevas); servicio non-root leyendo secrets; accounting/bridge/crons intactos | Phase 3 | cc:完了 [344644e + en vivo 2026-08-27: GRANT app_decide→orbit_admin, chown 10001, recreate verificado] |
| 4.2 | Seeds (dueño reactiva Exacts US ANTES): goals harvest MX + 2 goals campaign US (ids ad_entity verificados post-sync, destinos ENABLED verificado), caps día 1, ventana 48h, veto 30d. `[tdd:skip:seed-config]` | SELECTs en vivo verifican todo; goal_harvest_completo satisfecho; destino PAUSED rechazado por el DoD; fail-closed sin clave probado | 4.1 | cc:完了 [en vivo 2026-08-27: terna harvest goal 4 (Arras Manual 2.50 MXN) + goals 6/7 campaign US + caps día 1 (config id 7); divergencia decisión 15 resuelta por el dueño — brief manda, USPerNog PAUSED; evidencia out/orbit-04-4-2-seeds-20260827.md] |
| 4.3 | Ensayo E2E pre-cutover con `tools/smoke_apply.py` (re-verificación de 2.5 contra el deploy real) + el dueño ejecuta UN VETO real sobre una fila shadow de la cola (prerequisito de ORBIT 05). `[tdd:skip:smoke-produccion]` | Evidencia en ORBIT 04; estado final == inicial; veto del dueño registrado con su actor | 4.2 | cc:完了 [corrida 2026-08-28: 4/4 formas neto-cero (bid_keyword/negative/keyword en A USPerNog Category Exact; bid_target en B USPerNog Auto Discovery), doble readback, ledger probe 22-29 quota exenta; config 8/9 sembradas y 10 de cierre limpia; SHAPEs re-confirmados. VETO: fila 3 (harvest "arras matrimoniales cristianas") vetada por el endpoint real con actor 'gon' — ejecutado por el lead por delegación expresa del dueño (declarado); **VETO PERSONAL DEL DUEÑO CUMPLIDO 2026-08-28 06:54 UTC: fila 4 (harvest "arras matrimoniales personalizadas") vetada por el dueño con su propia mano, actor 'gon-personal', vence 2026-09-27 — verificado en apply_queue por el lead**. DoD literal satisfecho (prerequisito de ORBIT 05). Evidencia out/orbit-04-4-3-ensayo-e2e-20260828.md + out/smoke-apply-20260828.log] |
| 4.4 | Cierre: backup pre-cutover (ad_entity_state + keywords/negativeKeywords/targets lists), CHAT-CONTEXT al día, PR final, `ORBIT 04` Done. (El veto personal del dueño ya quedó cumplido en 4.3: fila 4, actor 'gon-personal'.) ORBIT 05 = backup pre-cutover REAL → discard masivo de filas shadow (app_admin) → flip → rampa (checklist §12 del brief; el backup del 28 fue el ENSAYO del runbook). `[tdd:skip:cierre-docs]` | Backup verificado; CI verde; escalera en `shadow` verificada; **checkpoint humano: spot-check manual del dueño de ≥20 decisiones del shadow — tabla preparada por el implementador (GLM, autor del motor — NO es verificación independiente) + 11 filas re-calculadas por el lead, 33 decisiones / 0 divergencias, en AppFlowy "ORBIT 04 4.4 — spot-check" y en `out/orbit-04-4-4-cierre-20260828.md` §3; columna "validado por el dueño" SIN firmar: `ORBIT 04` Done en AppFlowy SOLO al firmar**; neto-cero de 4.3 re-verificado contra el cache POST-sync (synced_at 2026-08-28 06:45 UTC posterior a la corrida); la verificación adversarial TRIPLE (codex+grok+qwen) de las primeras decisiones APLICADAS EN VIVO se MOVIO al checklist §12 de APPLY.md (es ritual de ORBIT 05, no de 4.4 — corrección del commit 1e41a1f); evidencia completa en `out/orbit-04-4-4-cierre-20260828.md` | 4.3 | cc:完了 [2026-08-28: backup pre-cutover verificado VERIFY_OK en `backups/precutover_orbit04_2026-08-28/` (dump+globals+CSV ad_entity_state+listas Amazon); spot-check 33 decisiones 0 divergencias — su FIRMA del dueño quedó como ítem 3 SIN MARCAR del checklist §12 (candado pre-flip); escalera shadow verificada; corrección 1e41a1f aplicada. POST-CIERRE (dueño, 2026-08-28, del spot-check): goal 4 MX floor/ceiling 1.00/45.00 MXN (el default 2.50 era un número en USD aplicado a pesos — verificado sin mezcla de monedas); umbrales de pausa 100 clics / 40 USD (MX 500 MXN) → tarea CORTES 03 antes del flip (CORTES 02 es la lista curada de términos)] |
