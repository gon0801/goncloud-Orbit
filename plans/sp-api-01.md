...
Version: 1.3, 2026-09-10 UTC. Estado: **Fase 0 y Fase A CERRADAS** (A.1–A.5 mergeadas, A.R con APPROVE de Grok sobre `958c00f`, A.6 desplegada en produccion sobre `aee0221` con cron diario instalado). D1–D7 cerradas por el dueño 2026-09-09 UTC. Fase 0 verificada 2026-09-09 UTC: sondas y actas en `docs/evidencia/sp-api-01/0.1–0.5/` (PR #236, Muse; validadas por el lead).
...
| A.2 | [stage:implementacion] [lane:gate] [tdd:required] Ingesta Orders append-only sin PII (clave definida en E/0.1 antes de A.2; candidata: identificador de orden de Amazon + `LastUpdateDate` + `observed_at`); paginacion completa con `NextToken` repetido y pagina vacia como tests | pytest_focal pasa; clave documentada en E/0.1; re-corrida no duplica (idempotencia probada contra esa clave); E/A.2 con conciliacion contra la muestra de 0.1 | A.1 | cc:完了 [2026-09-09 (lead): PR #239 — migración 0030 `spapi_order_observation` (clave (platform, amazon_order_id, last_updated_time), dinero con moneda, trigger purchase<=updated, append-only por prohibir_mutacion, grants mínimos con asserts negativos); `app/spapi/orders.py`: Orders 2026-01-01 sin PII, ventana lastUpdatedAfter=max−1d (primera createdAfter 30d, Zulu), paginación con guardas, contrato estricto (sin lista orders = ok=false), token bucket 20/0.0056; CLI `ingest spapi_orders --platform`; e2e idempotente en Postgres (CI); fixtures largos + `register_secret` ignora <8 chars (rompía tests ajenos). Mutaciones verificadas por el lead. E/A.2. **A.2b (PR #240, mergeado 2026-09-09):** pedidos con estado y total vía secciones FULFILLMENT + PROCEEDS (jamas BUYER/RECIPIENT), migración 0031 (clave bitemporal de 4 columnas `(platform, amazon_order_id, last_updated_time, observed_at)`, columna `fulfillment_status`, vista `v_spapi_order_ultima`, reversa con guarda doble), backfill `--desde`; rondas del lead y de CodeRabbit atendidas (respaldo de moneda por None, sello con detalle de skips, COMMENT refrescado, tests que muerden). Desplegado en el A.6 parcial (2026-09-10: 0030+0031 aplicadas en producción, corridas reales ok)] |
| A.3 | [stage:implementacion] [lane:gate] [tdd:required] Ingesta Pricing append-only por (ASIN, mercado); dinero `(valor, moneda)`, precio sin moneda = fila no escrita | pytest_focal pasa; fixture sin moneda no escribe fila; E/A.3 conciliada contra 0.2 | A.1 | cc:完了 [2026-09-09 (lead): PR #241 — migraciones 0032 `spapi_price_observation` (clave (asin, platform, observed_at), dinero NUMERIC+enum, trigger `spapi_price_tiempo_coherente` con UTC fijado: metric_date = dia UTC de observed_at, append-only por prohibir_mutacion, grants minimos) y 0033 (`ingest_run.llamadas`); `app/spapi/pricing.py`: pase diario por ASIN propio de `listing` (MX+US), offers + competitivePrice a 0.5/s con cubo compartido `CuboTasa`, 0 ofertas = fila con conteos 0 y precios NULL, precio sin moneda = fila no escrita + rows_skipped, CLI `ingest spapi_pricing --platform`; `register_secret` sin piso (cambio global declarado en el PR). Rondas del lead y CodeRabbit atendidas (conteo FBA solo condicion New, except redundante, premisas documentadas). E/A.3. Desplegado en el A.6 parcial (2026-09-10: 0032-0034 aplicadas en producción, runs 148/149 ok=true)] |
| A.4 | [stage:implementacion] [lane:gate] [tdd:required] Ingesta Listings Items + Inventario FBA append-only (D2=A cerrada: SP-API no pisa precio/stock del bridge; solo Buy Box, competencia y estado del listing; ninguna escritura de precio ni stock en `listing`) | pytest_focal pasa; test de que precio/stock SP-API no sobrescribe `listing`; E/A.4 conciliada contra 0.3/0.4 | A.1 | cc:完了 [2026-09-10 (lead): PR #244 (Muse) — migración 0035 (`spapi_listing_estado_observation` y `spapi_inventario_observation`, append-only por (entidad, platform, observed_at), trigger `spapi_inventario_tiempo_coherente` con UTC fijado, GRANTs a nivel tabla); `app/spapi/listings.py` (universo seller_sku de `listing`, 5/s, status TEXT tolerante declarado) e `inventario.py` (pagination hermana de payload, 2/s, guardas de orders); `VENDEDORES_PROPIOS` movido a client.py sin cambio de comportamiento; test D2 con snapshot de las 7 columnas de `listing`; mutación del lead demostrada (regla 9). E/A.4. Pendientes BAJA del review: conteos de líneas de la evidencia (479/445 vs 504/469 reales), `asins_vistos` cuenta SKUs, nombre de `test_muro_aborta_con_lo_escrito`, SELECT de cortesía del trigger en el checklist del deploy de 0035. Deploy en el A.6 final] |
| A.5 | [stage:implementacion] [lane:gate] [tdd:required] Salud en `/salud` + alertas via `app/notifica.py` (fallo sostenido, 429 persistente, refresh LWA fallido — este ultimo no tumba el ciclo de Ads) | pytest_focal pasa; refresh LWA caido deja traza en `/salud` y el ciclo de Ads sigue; E/A.5 | A.2–A.4 | cc:完了 [2026-09-10 (lead): entro en DOS PRs. PR #246 (migracion 0036, `ingest_run.platform`) se mergeo con 5 bloqueantes vivos que hallo la review del lead; el arreglo es PR #247 (merge 542564d, migracion 0037: indice de `/salud` + candado del INSERT). Bloqueantes: (1) `evaluar_alertas` dejaba la conexion INTRANS y degradaba el sello siguiente a savepoint (riesgo de repetir las corridas huerfanas 145/147); (2) las alertas inmediatas sin flanco (8 mensajes diarios con LWA caido); (3) el candado del GRANT probaba el INSERT como dueno y no como `app_ingest` (clase 0033->0034); (4) el historial contaba corridas abiertas y no anclaba la corrida sellada; (5) cableado sin probar en 3 de las 4 ingestas. Verificacion por mutacion: 29 mutantes sobre la entrega original (21 sobrevivieron) y 13 sobre la ronda (12 mueren). CI verde sobre 542564d. Semantica final: una alerta por racha; inmediata en `lwa_fallido`/`http_429` solo si la anterior no era de esa misma clase; el cambio de clase dentro de una racha si re-alerta. E/A.5. NO desplegado: produccion sigue en 0031-0034] |
| A.R | [stage:revision] [lane:gate] [tdd:skip:revision] Revision independiente de A | Reviewer devuelve APPROVE sobre SHA concreto (guard, un solo refrescador, append-only, dinero, redaccion, sin PII); ningun hallazgo bloqueante abierto; E/A.R enlaza informe | A.5 | cc:完了 [2026-09-10 (Grok, revision independiente): **APPROVE sobre `958c00f`**, informe en `docs/evidencia/sp-api-01/A.R/informe.md`, brief en `plans/brief-sp-api-01-ar-grok.md`. Los seis ejes cumplen (guard default-deny, un solo refrescador LWA, append-only + GRANTs por columna, dinero con moneda sellada, redaccion, sin PII). 18 mutantes: 14 mueren, 4 sobreviven (declarados, ninguno bloqueante). Cero hallazgos bloqueantes. Dos hallazgos NO bloqueantes, ambos atendidos en este mismo PR: **H1** la propuesta de cron no era instalable (ocho lineas de Vixie que no encadenaban, colchon de ~2 min en pricing MX, y el instalador de ORBIT 03 las borraba en silencio por su `grep -v "app.cli ingest"`) -> reescrita como wrapper en serie con `flock`, logs y `job_key` propio; **H2** ningun test afirmaba el VERBO del camino de lectura (el mutante `client.get`->`client.post` sobrevivia a 57 tests) -> `test_camino_de_lectura_sale_como_GET`, verificado que ahora mata ese mutante. Verificacion del lead sobre el informe: reproduje de forma independiente las dos afirmaciones de mayor consecuencia (el filtro del instalador de Ads y el mutante M15) y las dos se sostienen] |
| A.6 | [stage:cierre-pr] [lane:release] [tdd:skip:validacion-entrega] CI, integracion y despliegue A | Ruff/pre-commit pasan; PR con suite completa verde; migracion `00NN` aplicada con backup; smoke de lectura sin mutaciones; reversa ensayada (apagar ingesta); E/A.6 con SHA y resultados | A.R | cc:完了 [2026-09-10 (dueno, con `!`): **DEPLOY FINAL sobre `aee0221`** con `out/migrar-a6-final.sh`. Respaldo `schema-pre-a6final-20260910-0759.sql` (45 CREATE TABLE); `0035`->`0036`->`0037` aplicadas en transaccion con ON_ERROR_STOP; verificacion como orbit_read (5 triggers, ingest_inserta=t / read_inserta=f / ingest_actualiza=f, INSERT en platform=t y UPDATE=f, indexdef `(source, platform, id DESC)`); deploy md5 99/99, Recreated, health ok; cron instalado como wrapper EN SERIE + una linea con `flock` y log (crontab 47->49, aditivo) que ya NO calza los filtros del instalador de ORBIT 03 (hallazgo H1 de A.R). Primeras corridas reales MX: run 154 orders 13 escritas, run 155 listings 342, run 156 inventario 1071 — las tres ok=true, 0 omitidas, `platform` sellada; conteos de tabla identicos a lo reportado por cada run; los pipelines viejos siguen con platform NULL, sin romperse. Smoke de candados con datos dentro: UPDATE sobre las dos tablas nuevas revienta append-only y hace ROLLBACK (1071 intacto). Reversa ensayada punta a punta: apagar la ingesta deja 0 lineas spapi y restaurar vuelve a 2, 49 totales sin tocar el resto. E/A.6. **Sin ejercitar todavia**: pricing (~23 min), toda la plataforma US, el wrapper como tal y el camino de alertas — todo eso lo estrena el cron de las 05:00 UTC. Historico: deploy de A.1+A.2/A.2b+A.3 a produccion — migraciones 0031-0034 aplicadas con backup verificado, app reconstruida desde origin/master (md5 identicos, db intacta), primeras corridas reales: orders ok (runs 143/144) y pricing ok tras el fix del GRANT de `ingest_run.llamadas` (bug de produccion: PR #242, 0034; runs 148/149 ok=true, universo completo 342 MX + 176 US; corridas huerfanas 145/147 selladas a mano con conteos conciliados). E/A.6. Falta el deploy final tras A.4/A.5 con su cron] |
...
- Fase A, **todo el código mergeado**: A.1 (PR #237), A.2 (PR #239), A.2b (PR #240), A.3 (PR #241), hotfix GRANT (PR #242, migración 0034), A.4 (PR #244, migración 0035) y A.5 (PR #246 + su ronda de correcciones PR #247, migraciones 0036 y 0037). **Deploy parcial A.6 hecho 2026-09-10 UTC**: 0031-0034 aplicadas en producción con backup, app reconstruida, primeras corridas reales de orders y pricing selladas ok (E/A.6). **A.R cerrada 2026-09-10**: Grok devolvió APPROVE sobre `958c00f` (informe en `docs/evidencia/sp-api-01/A.R/`), sin hallazgos bloqueantes; sus dos hallazgos menores (cron no instalable, verbo GET sin pinear) quedaron corregidos en el PR de cierre. **A.6 cerrada el mismo día**: `aee0221` desplegado en producción con `0035`/`0036`/`0037` aplicadas con respaldo, cron diario instalado como wrapper en serie con `flock`, y primeras corridas reales de MX (runs 154/155/156, las tres ok, 0 omitidas). Fase A completa. Lo que estrena el cron de las 05:00 UTC —y por tanto sigue sin ejercitarse en producción— es `spapi_pricing`, toda la plataforma US, el wrapper como tal y el camino de alertas de A.5.
